...
void porcentajeTest(){
    if(nroTotalPreguntas==0){
        exit(-1);
    }
    printf("%d\n",nroPreguntasCorrectas);
    printf("%d\n",nroTotalPreguntas);
    float res=(float)nroPreguntasCorrectas/(float)nroTotalPreguntas;
    printf("Porcentaje de respuestas correctas respecto al total de preguntas es de %2.0f%c\n",res*100,'%');
    if(res<0.75){
